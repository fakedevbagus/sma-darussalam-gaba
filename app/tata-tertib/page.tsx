...
export default function TataTertibPage() {
  ...
  return (
    <div>
      <PageHeader badge="KESISWAAN • TATA TERTIB" title="Tata Tertib" accent="Siswa" desc="Peraturan yang berlaku untuk seluruh siswa SMA Biru Ceria — demi kedisiplinan & kenyamanan bersama." img="https://images.unsplash.com/photo-1503676260728-1c00da094a0b?q=80&w=800&auto=format&fit=crop" breadcrumb="Kesiswaan / Tata Tertib" />
      <section className="max-w-[1280px] mx-auto px-6 -mt-2 space-y-6">
        <div className="bg-white rounded-[32px] p-8 shadow-card border border-slate-100 flex gap-4 items-center">
        ...
        <div className="space-y-4">
          {TATA_TERTIB.categories.map((cat, idx) => {
            ...
            return (
              <div key={idx} className="bg-white rounded-[24px] shadow-card border border-slate-100 overflow-hidden">
                ...
                {isOpen && <div className="px-6 pb-6 pt-2 border-t border-slate-100"><ol className="space-y-2">{cat.rules.map((r,i)=> (<li key={i} className="flex gap-3 text-sm text-slate-600"><span className="w-1.5 h-1.5 bg-primary-600 rounded-full mt-2 shrink-0" /><span>{r}</span></li>))}</ol></div>}
              </div>
            );
          })}
        </div>

        ...
        <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-5xl mx-auto">
          {filtered.map(doc=> (
            <div key={doc.id} className="bg-white rounded-[24px] p-5 shadow-card border border-slate-100 hover:shadow-3d hover:-translate-y-1 transition flex flex-col">
              ...
              <button disabled className="mt-4 w-full bg-slate-100 text-slate-500 py-2.5 rounded-xl text-xs font-bold flex gap-2 justify-center items-center cursor-not-allowed"><Download className="w-3.5 h-3.5" /> Segera Tersedia (Demo)</button>
              <div className="text-[10px] text-slate-400 mt-2 text-center">Ganti URL di lib/demo-data.ts untuk aktifkan unduh</div>
            </div>
          ))}
        </div>

        <p className="text-center text-xs text-slate-500 mt-8">Semua dokumen demo — hubungi admin via <a href="/kontak" className="text-primary-600 font-bold underline">Kontak</a> untuk file resmi.</p>
      </section>
    </div>
  );
}

...
const MILESTONES = [
  { year: "2005", title: "Pendirian Sekolah", desc: "SMA Biru Ceria didirikan Yayasan Pendidikan Cendekia — 3 kelas, 90 siswa di Kebayoran.", icon: Calendar },
  { year: "2012", title: "Akreditasi A Pertama", desc: "Raih akreditasi A BAN-SM, bangun lab IPA terpadu.", icon: Award },
  { year: "2015", title: "Lab Komputer 60 Unit", desc: "Studio robotik & digital lab — era komputerisasi.", icon: Lightbulb },
  { year: "2019", title: "Go Internasional", desc: "Kerjasama Cambridge & student exchange Singapura.", icon: Target },
  { year: "2024", title: "Prestasi Nasional", desc: "Emas OSN Matematika & Juara KRI Regional.", icon: Trophy },
];

export default function SejarahPage() {
  return (
    <div>
      <PageHeader badge="PROFIL • SEJARAH" title="Sejarah" accent="Perjalanan Kami" desc="Tonggak penting SMA Biru Ceria sejak berdiri hingga menjadi sekolah favorit Jakarta Selatan." img="https://images.unsplash.com/photo-1461360228754-6e81c478b882?q=80&w=800&auto=format&fit=crop" breadcrumb="Profil / Sejarah" />
          ...
          <div className="space-y-6">
            {MILESTONES.map((m, i) => (
              <div key={m.year} className="relative flex gap-4">
                ...
                <div className="flex-1 bg-white rounded-2xl border border-slate-100 p-5 shadow-card hover:shadow-3d transition">
        ...
        <div className="mt-12">
          ...
          <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-4 gap-4 max-w-4xl mx-auto">
            {SCHOOL.stats.map((s, i) => (
              <div key={s.label} className="bg-white rounded-[20px] p-6 shadow-card border border-slate-100 text-center card-3d">

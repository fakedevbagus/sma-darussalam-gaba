    ...
    <div>
      ...
      <section className="max-w-[1280px] mx-auto px-6 -mt-2">
        ...
        <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {items.map((item,i)=> (
            <button key={item.id} onClick={()=>setIdx(i)} className={`group relative text-left bg-white rounded-[24px] p-3 pb-5 shadow-card border border-slate-100 hover:shadow-3d hover:-translate-y-1 transition overflow-hidden ${i%4===0?"rotate-[0.5deg]": i%4===1?"-rotate-[0.5deg]": i%4===2?"rotate-[0.3deg]":"-rotate-[0.3deg]"}`}>
        ...
        {viewing && (
          <div className="fixed inset-0 z-50 bg-navy/80 backdrop-blur flex items-center justify-center p-4" onClick={()=>setIdx(null)}>
            <div className="bg-white rounded-[24px] overflow-hidden max-w-3xl w-full shadow-3d" onClick={e=>e.stopPropagation()}>
              <div className="relative bg-black">
                {viewing.videoUrl ? <iframe src={viewing.videoUrl} className="w-full aspect-video" allowFullScreen title={viewing.title} /> : <img src={viewing.imageUrl} alt={viewing.title} className="w-full max-h-[65vh] object-contain" />}
        ...
        <div className="mt-10 bg-white rounded-[32px] p-8 shadow-card border border-slate-100">
          <h3 className="font-extrabold text-navy">Video Pilihan</h3>
          <div className="mt-6 grid md:grid-cols-3 gap-6">
            {GALLERY.filter(g=>g.videoUrl).slice(0,3).map(v=> (
              <button key={v.id} onClick={()=> setIdx(GALLERY.findIndex(x=>x.id===v.id))} className="rounded-[24px] overflow-hidden bg-navy text-white relative aspect-video flex items-center justify-center group">
                <img src={v.imageUrl} alt={v.title} className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:scale-105 transition duration-700" />
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/70 to-transparent text-left">
                  ...
                  <div className="text-xs text-white/70">YouTube • SMA Biru Ceria</div>
                </div>
              </button>
            ))}
          </div>
